...
const productSchema = z.object({
  ...
  price: z.string()
    ...
    .refine((val) => /^\d+(\.\d{0,2})?$/.test(val), "Invalid format. Use up to 2 decimal places.")
    .refine((val) => {
  ...
  quantity: z.preprocess((val) => String(val), z.string()
    ...
    .refine((val) => /^[1-9]\d*$/.test(val), (val) => {
      if (val === '0') return { message: "Quantity must be greater than zero." };
      if (val.startsWith('0')) return { message: "Leading zeros are not allowed." };
      return { message: "Quantity must be a positive whole number." };
    })),
...
const ManufacturerProducts = () => {
  ...
  const handleEditChange = (event) => {
    ...
    if (name === 'price') {
      // Block scientific notation and other symbols, allow only numbers and one decimal point
      value = value.replace(/[eE+\-]/g, "");
      
      // Prevent more than 2 decimal places as user types
      if (value.includes('.')) {
  ...
  const handleUpdateClick = () => {
    ...
    if (original) {
      const isChanged = 
        original.name !== editProduct.name ||
        original.category !== editProduct.category ||
        original.price !== editProduct.price ||
        String(original.quantity) !== String(editProduct.quantity) ||
        original.warranty !== editProduct.warranty ||
        original.description !== editProduct.description ||
        original.is_active !== editProduct.is_active;
          ...
          <div className="product-modal" onClick={(event) => event.stopPropagation()}>
            ...
            <form className="product-form" onSubmit={(e) => { e.preventDefault(); handleUpdateClick(); }}>
              ...
              <div className="product-grid">
                ...
                <label className="product-field">
                  <span>Quantity</span>
                  <input type="text" name="quantity" value={editProduct.quantity} onChange={handleEditChange} />
          ...
          <div className="product-modal" onClick={(event) => event.stopPropagation()}>
            ...
            <form className="product-form" onSubmit={handleAddSubmit}>
              ...
              <div className="product-grid-three">
                ...
                <label className="product-field">
                  <span>Quantity</span>
                  <input type="text" name="quantity" value={newProduct.quantity} onChange={handleAddChange} />
                  {errors.quantity && <span className="field-error">{errors.quantity[0]}</span>}
                </label>
                <label className="product-field">
                  <span>Warranty Type</span>
